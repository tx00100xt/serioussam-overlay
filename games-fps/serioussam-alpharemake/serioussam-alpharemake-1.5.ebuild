...
src_install() {
	...
	cp -fr "${dir}"/Scripts/CustomOptions/GFX-AdvancedRendering.cfg \
		"${D}${dir}/Mods/${MY_MOD}"/Scripts/CustomOptions || die "Failed to copy /GFX-AdvancedRendering.cfg"
	cp -fr "${FILESDIR}"/GFX-RenderingOptions.cfg \
		"${D}${dir}/Mods/${MY_MOD}"/Scripts/CustomOptions || die "Failed to copy GFX-RenderingOptions.cfg"
