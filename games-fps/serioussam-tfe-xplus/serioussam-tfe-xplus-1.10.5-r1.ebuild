...
src_install() {
	...
	if use x86; then
		...
		mv "${BUILD_DIR}"/Debug/libEntities.so "${D}/usr/lib/${GN}/Mods/XPLUS" || die "Failed moved libEntities.so"
		mv "${BUILD_DIR}"/Debug/libGame.so "${D}/usr/lib/${GN}/Mods/XPLUS" || die "Failed moved libGame.so"
		# dosym "/usr/lib/${GN}/libamp11lib.so" "/usr/lib/${GN}/Mods/XPLUS/libamp11lib.so"
	else
		mkdir "${D}/usr/lib64/${GN}/Mods/XPLUS"
		mv "${BUILD_DIR}"/Debug/libEntities.so "${D}/usr/lib64/${GN}/Mods/XPLUS" || die "Failed moved libEntities.so"
		mv "${BUILD_DIR}"/Debug/libGame.so "${D}/usr/lib64/${GN}/Mods/XPLUS" || die "Failed moved libGame.so"
