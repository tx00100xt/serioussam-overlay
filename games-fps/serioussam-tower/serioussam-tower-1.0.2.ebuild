...
SRC_URI="${URL}SE1-TFE-${MY_MOD}/archive/refs/tags/v${PV}.tar.gz -> ${P}.tar.gz
	${URL}serioussam-mods/raw/main/SamTFE-${MY_MOD}/SamTFE-${MY_MOD}.tar.xz"
S="${WORKDIR}/SE1-TFE-${MY_MOD}-${PV}/Sources"

MY_CONTENT="${WORKDIR}/SE1-TFE-${MY_MOD}-${PV}/${MY_PN}"
MY_MOD_ARC="SamTFE-${MY_MOD}.tar.xz"
...
src_install() {
	...
	# moving libs
	if use x86; then
		mv "${BUILD_DIR}"/Debug/libEntities.so \
			"${D}/usr/lib/${GN}/Mods/${MY_MOD}" \
				|| die "Failed to moved libEntities.so"
		mv "${BUILD_DIR}"/Debug/libGame.so \
			"${D}/usr/lib/${GN}/Mods/${MY_MOD}" \
